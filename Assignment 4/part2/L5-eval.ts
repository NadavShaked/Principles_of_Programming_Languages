...
import { map, repeat, zipWith, pair } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, ValuesExp, LetValuesExp, isLetValuesExp } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, makeTupleSExp, isTupleSExp } from "./L5-value";
import { isEmpty, first, rest } from '../shared/list';
import { Result, makeOk, makeFailure, mapResult, safe2, bind, zipWithResult } from "../shared/result";
...
const evalValues = (exp: ValuesExp, env: Env): Result<Value> =>   // new
    bind(mapResult((exp: CExp) => applicativeEval(exp, env), exp.vals), (exps: Value[]) => makeOk(makeTupleSExp(exps)));
...
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    ...
    const cvals: Result<Value[]> = mapResult(val => applicativeEval(val, env), vals);

    const pairs: Result<[VarDecl, Value][][]> = bind(cvals, vals => zipWithResult((vars, val) => {
        if (!isTupleSExp(val))
            return makeFailure("Value isn't Tuple");
        if (val.vals.length !== vars.length)
            return makeFailure("The values lenght dont match");
        return zipWithResult((r, l) => makeOk([r, l]), vars, val.vals);
    } , varss, vals))

    return bind(pairs, x => {
        const p = R.chain(R.identity, x);
        const vars = R.map(x => x[0], p);
        const names = map((x: VarDecl) => x.var, vars);
        const values = R.map(x => x[1], p);
        return evalSequence(exp.body, makeExtEnv(names, values, env))
    })
}