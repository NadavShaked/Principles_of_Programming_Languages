...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, VarDecl, CExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, isNonTupleTExp, isTupleTExp, isEmptyTupleTExp } from "./TExp";
...
import { makeTupleSExp, Value, isTupleSExp } from './L5-value';
...
export const typeofValues = (exp: ValuesExp, tenv: TEnv): Result<TExp> => {
    const varsType = mapResult(e => typeofExp(e, tenv), exp.vals);
    return isEmpty(exp.vals)? makeOk(makeEmptyTupleTExp()) :
                            bind(varsType, x => allT(isNonTupleTExp, x)? makeOk(makeNonEmptyTupleTExp(x)) : makeFailure("tuple in tuple"));
}

export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    const varss: VarDecl[][] = map(bdg => bdg.vars, exp.valueBindings);
    const vals: CExp[] = map(bdg => bdg.val, exp.valueBindings);
    const cvals: Result<TExp[]> = mapResult(val => typeofExp(val, tenv), vals);

    const pairs: Result<[VarDecl, TExp][][]> = bind(cvals, (texps) => zipWithResult((vars, val) => {
        if (!isTupleTExp(val))
            return makeFailure("Value isn't Tuple");
        return isEmptyTupleTExp(val)? makeFailure("empty texp")/*todo*/ :
        (val.TEs.length !== vars.length) ? makeFailure("The values lenght dont match") :
        zipWithResult((r, l) => makeOk([r, l]), vars, val.TEs);
    } , varss, texps))

    return bind(pairs, x => {
        const p = R.chain(R.identity, x);
        const vars = R.map(x => x[0], p);
        const varsNames = map((x: VarDecl) => x.var, vars);
        const varsTypes = map((x: VarDecl) => x.texp, vars);
        const values = R.map(x => x[1], p);

        const constraints = zipWithResult((varType, varVal) => checkEqualType(varType, varVal, exp), varsTypes, values);
        return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(varsNames, varsTypes,tenv)));
    })
}