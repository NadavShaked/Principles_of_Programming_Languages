...
//         |  ( values <cexp>+)               / ValuesExp(vals: CExp[]) //new
//         |  ( let-values ( <value-binding>* ) <cexp>+)  / LetValuesExp(value-binding: ValueBinding[], body: CExp[])   // new
// <binding>  ::= ( <var> <cexp> )            / Binding(var:VarDecl, val:Cexp)
// <value-binding> :: ( <var>* <cexp> )       / ValueBinding(vars: VarDecl[], vals: Cexp)   // new
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | ValuesExp | LetValuesExp;  //new
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isValuesExp(x) || isLetValuesExp(x); //new
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isValuesExp(e) ? e.vals :   //new
    isLetValuesExp(e) ? [...e.body, ...map((b) => b.val, e.valueBindings)] :    // new
    isDefineExp(e) ? [e.val] :
...
export interface ValueBinding {tag: "ValueBinding"; vars: VarDecl[]; val: CExp; }
export const makeValueBinding = (v: VarDecl[], val: CExp): ValueBinding =>
    ({tag: "ValueBinding", vars: v, val: val});
export const isValueBinding = (x: any): x is ValueBinding => x.tag === "ValueBinding";

export interface LetValuesExp {tag: "LetValuesExp"; valueBindings: ValueBinding[]; body: CExp[]}
export const makeLetValuesExp = (bindings: ValueBinding[],b: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", valueBindings: bindings, body: b });
...
export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    op === "values" ? parseValuesExp(params) :  // new
    ...
    op === "let-values" ? parseLetValuesExp(first(params), rest(params)) :   // new
    makeFailure("Never");
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body of "let" cannot be empty') :
    ! isGoodBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    safe2((bdgs: ValueBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
        (parseValueBindings(bindings), mapResult(parseL5CExp, body));

const parseValueBindings = (bindings: [Sexp, Sexp][]): Result<ValueBinding[]> => {
    const varDeclArray = map((b => b[0]),bindings);
    const varDecl2DArr = mapResult((v) => isArray(v)? mapResult(parseVarDecl, v): mapResult(parseVarDecl, [v]) ,varDeclArray);
    const val = map((b) => b[1], bindings);
    const valAsCexp = mapResult(parseL5CExp, val);
    return safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeValueBinding, vds, vals)))
            (varDecl2DArr, valAsCexp);
}
...
export const parseValuesExp = (param: Sexp[]): Result<ValuesExp> => // new
    isEmpty(param) ? makeOk(makeValuesExp([])) :
    bind(mapResult(parseL5CExp, param), (cexps: CExp[]) => makeOk(makeValuesExp(cexps)));
...
const unparseValuesExp = (v: ValuesExp) : Result<string> =>
    bind(mapResult(unparse, v.vals), (s) => makeOk(`(values ${s.join(" ")})`));    

const unparseLetValuesExp = (lv: LetValuesExp) : Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (bind(mapResult(unparseValueBindings, lv.valueBindings), (arr) => makeOk(arr.join(" "))), unparseLExps(lv.body));

//vars: VarDecl[]; val: CExp;
const unparseValueBindings = (valB: ValueBinding): Result<string> =>
        safe2((vars: string, val: string) => makeOk(`(${vars} ${val})`))
            (bind(mapResult(unparseVarDecl, valB.vars), (arr) => makeOk(`(${arr.join(" ")})`)), unparse(valB.val));