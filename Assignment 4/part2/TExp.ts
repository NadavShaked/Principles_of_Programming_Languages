...
;; [Empty -> void]
*/
import { chain, concat, map, uniq, reduce } from "ramda";
...
import { TupleSExp, makeTupleSExp } from "./L5-value";
...
//    return (pos === -1) ? bind(parseTupleTExp(texps),ResTe => allT(isNonTupleTExp, ResTe) ? makeOk(makeNonEmptyTupleTExp(ResTe)) : makeFailure(`elements type are not NonTupleTExp -> - ${texps}`)) :
...
const parseCompoundTExp = (texps: Sexp[]): Result<CompoundTExp> => {
    const pos = texps.indexOf('->');
   return (pos === -1) ? isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()) : bind(parseTupleTExp(texps),ResTe => allT(isNonTupleTExp, ResTe) ? makeOk(makeNonEmptyTupleTExp(ResTe)) : makeFailure(`elements type are not NonTupleTExp -> - ${texps}`)) :
           (pos === 0) ? makeFailure(`No param types in proc texp - ${texps}`) :
           (pos === texps.length - 1) ? makeFailure(`No return type in proc texp - ${texps}`) :
           (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${texps}`) :
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
