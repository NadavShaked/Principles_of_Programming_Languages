...
export interface TupleSExp {
    tag: "TupleSExp";
    vals: SExpValue[];
}

export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp | void | TupleSExp;
export const isSExp = (x: any): x is SExpValue =>
    typeof(x) === 'string' || typeof(x) === 'boolean' || typeof(x) === 'number' ||
    isSymbolSExp(x) || isCompoundSExp(x) || isEmptySExp(x) || isPrimOp(x) || isClosure(x) || isTupleSExp(x);    // new
...
export const makeTupleSExp = (values: SExpValue[]): TupleSExp =>
    ({tag: "TupleSExp", vals: values});
export const isTupleSExp = (x: any): x is TupleSExp => x.tag === "TupleSExp";
...
export const tupleSExpToString = (tuple: TupleSExp): string =>  // new
    `<Tuple ${join(',', map(valueToString, tuple.vals))}>`
...
export const valueToString = (val: Value): string =>
    ...
    isEmptySExp(val) ? "'()" :
    isTupleSExp(val) ? tupleSExpToString(val) :
