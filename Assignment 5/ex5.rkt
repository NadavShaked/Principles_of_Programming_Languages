...
(define sqrt2
  (lambda (x init epsilon)
    (car (find-first (sqrt-lzl x init) (lambda (guess) (good-enough? (car guess) x epsilon))))
  )
)
...
(define collect-all-values-2
 (lambda (lists key)
    (if (empty? lists)
      '()
      (if (eq? (get-value$ (car lists) key (lambda(x) x) (lambda() #f)) #f)
