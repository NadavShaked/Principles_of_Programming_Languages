...
describe('L4 Normal Eval', () => {
    ...
    it('our tests', () => {
        expect(bind(parseL4('(L4 (define func (lambda (x y) (if (> x y) (* x x) (function x)))) (func 7 5))'), evalNormalProgram)).to.deep.equal(makeOk(49))
        expect(bind(parseL4(`(L4 ((lambda (a b c) (if a b c)) #t 8 (/ 3 0)))`), evalNormalProgram)).to.deep.equal(makeOk(8));
        expect(bind(parseL4('(L4 (define func1 (lambda (func1))) (define func2 (lambda (x y) (if (= y 0) x y))) (func2 func1 25))'), evalNormalProgram)).to.deep.equal(makeOk(25))
    });
