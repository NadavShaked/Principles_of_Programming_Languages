...
describe("is Some/None", () => {
    ...
    describe("monadic validate user", () => {
        it("validates user data with bind",()=>{
            expect(monadicValidateUser(user1)).to.deep.equal({ tag: 'Ok',
             value: { name: 'Ben',
        ...
        it("validates user data with bind",()=>{
            expect(monadicValidateUser(user2)).to.deep.equal({  tag: 'Failure',
             message: 'Bananas is not a name'})
        })
        it("validates user data naively",()=>{
            expect(monadicValidateUser(user3)).to.deep.equal({  tag: 'Failure',
    ...
    describe("naive validate user", () => {
        it("validates user data  naively",()=>{
            expect(naiveValidateUser(user1)).to.deep.equal({ tag: 'Ok',
             value: { name: 'Ben',
             email: 'bene@post.bgu.ac.il',
             handle: 'bene' } })
        })
        ...
        it("validates user data naively",()=>{
            expect(naiveValidateUser(user2)).to.deep.equal({  tag: 'Failure',
             message: 'Bananas is not a name'})
        })

        it("validates user data naively",()=>{
            expect(naiveValidateUser(user3)).to.deep.equal({  tag: 'Failure',
             message: "Domain bananas.com is not allowed"})
    })
});